package sh.haven.app.agent

import android.content.Context
import io.mockk.coEvery
import io.mockk.coVerify
import io.mockk.every
import io.mockk.mockk
import kotlinx.coroutines.test.runTest
import org.json.JSONObject
import org.junit.Assert.assertEquals
import org.junit.Rule
import org.junit.Test
import org.junit.rules.TemporaryFolder
import sh.haven.core.data.font.TerminalFontInstaller
import sh.haven.core.data.preferences.UserPreferencesRepository
import sh.haven.core.data.repository.ConnectionRepository
import sh.haven.core.data.repository.PortForwardRepository
import sh.haven.core.ffmpeg.FfmpegExecutor
import sh.haven.core.ffmpeg.HlsStreamServer
import sh.haven.core.local.LocalSessionManager
import sh.haven.core.local.ProotManager
import sh.haven.core.rclone.RcloneClient
import sh.haven.core.ssh.SessionManagerRegistry
import sh.haven.core.ssh.SshSessionManager
import sh.haven.feature.sftp.SftpStreamServer
import sh.haven.feature.terminal.agent.TerminalSessionRegistry
import java.io.File

/**
 * #214 — chunked write_guest_file. A large file can be streamed in ordered
 * chunks instead of one huge base64 request:
 *  - first chunk {append:false, final:false} truncates the staging buffer
 *  - middle chunks {append:true,  final:false} append to it
 *  - last chunk  {append:true,  final:true}  copies the buffer into the guest
 * The guest `cp` (runCommandInProot) runs ONLY on the final chunk, and the
 * committed bytes are the concatenation of every chunk. Single-call usage
 * (no append/final) is unchanged.
 */
class McpChunkedWriteTest {

    @get:Rule
    val tmp = TemporaryFolder()

    private val targetPath = "/home/droid/big.bin"

    // Bytes the guest `cp` would have copied, captured per runCommandInProot
    // call (the staging file is deleted right after, so we read it inside
    // the stub before the finally block removes it).
    private val committed = mutableListOf<ByteArray>()

    private fun stagingFile(): File =
        File(tmp.root, "haven-write-" + Integer.toHexString(targetPath.hashCode()))

    private fun newTools(): Pair<McpTools, ProotManager> {
        val context = mockk<Context>(relaxed = true)
        every { context.cacheDir } returns tmp.root

        val proot = mockk<ProotManager>(relaxed = true)
        every { proot.isRootfsInstalled } returns true
        coEvery { proot.runCommandInProot(any()) } answers {
            val staged = stagingFile()
            committed.add(if (staged.exists()) staged.readBytes() else ByteArray(0))
            Pair("EXIT:0", 0)
        }
        val localSessionManager = mockk<LocalSessionManager>(relaxed = true)
        every { localSessionManager.prootManager } returns proot

        val tools = McpTools(
            context = context,
            connectionRepository = mockk<ConnectionRepository>(relaxed = true),
            portForwardRepository = mockk<PortForwardRepository>(relaxed = true),
            sshSessionManager = mockk<SshSessionManager>(relaxed = true),
            sessionManagerRegistry = mockk<SessionManagerRegistry>(relaxed = true),
            rcloneClient = mockk<RcloneClient>(relaxed = true),
            mailSessionManager = mockk<sh.haven.core.mail.MailSessionManager>(relaxed = true),
            sftpStreamServer = mockk<SftpStreamServer>(relaxed = true),
            hlsStreamServer = mockk<HlsStreamServer>(relaxed = true),
            ffmpegExecutor = mockk<FfmpegExecutor>(relaxed = true),
            preferencesRepository = mockk<UserPreferencesRepository>(relaxed = true),
            terminalFontInstaller = mockk<TerminalFontInstaller>(relaxed = true),
            localSessionManager = localSessionManager,
            agentUiCommandBus = sh.haven.core.data.agent.AgentUiCommandBus(),
            transportSelector = mockk<sh.haven.feature.sftp.transport.TransportSelector>(relaxed = true),
            workspaceRepository = mockk<sh.haven.core.data.repository.WorkspaceRepository>(relaxed = true),
            workspaceLauncher = mockk<sh.haven.app.workspace.WorkspaceLauncher>(relaxed = true),
            tunnelConfigRepository = mockk<sh.haven.core.data.repository.TunnelConfigRepository>(relaxed = true),
            tunnelManager = mockk<sh.haven.core.tunnel.TunnelManager>(relaxed = true),
            terminalSessionRegistry = TerminalSessionRegistry(),
            portKnocker = mockk<sh.haven.core.knock.PortKnocker>(relaxed = true),
            spaSender = mockk<sh.haven.core.spa.SpaSender>(relaxed = true),
            connectionLogRepository = mockk<sh.haven.core.data.repository.ConnectionLogRepository>(relaxed = true),
            servedFileTracker = sh.haven.core.data.agent.ServedFileTracker(),
            syncProfileRepository = mockk<sh.haven.core.data.repository.SyncProfileRepository>(relaxed = true),
            terminalInputQueue = mockk<TerminalInputQueue>(relaxed = true),
            prootInstallLogRepository = mockk<sh.haven.core.data.repository.ProotInstallLogRepository>(relaxed = true),
            sshKeyRepository = mockk<sh.haven.core.data.repository.SshKeyRepository>(relaxed = true),
            knownHostDao = mockk(relaxed = true),
            stepCaConfigRepository = mockk<sh.haven.core.data.repository.StepCaConfigRepository>(relaxed = true),
            totpSecretRepository = mockk<sh.haven.core.data.repository.TotpSecretRepository>(relaxed = true),
            ageIdentityRepository = mockk<sh.haven.core.data.repository.AgeIdentityRepository>(relaxed = true),
            desktopSessionRegistry = mockk<sh.haven.core.data.desktop.DesktopSessionRegistry>(relaxed = true),
            usbBroker = mockk<sh.haven.core.usb.UsbBroker>(relaxed = true),
            usbIpServer = mockk<sh.haven.core.usb.UsbIpServer>(relaxed = true),
            usbDriveVmManager = mockk<sh.haven.app.usb.UsbDriveVmManager>(relaxed = true),
            umlRecoveryManager = mockk<sh.haven.app.usb.UmlRecoveryManager>(relaxed = true),
            presentationManager = sh.haven.core.data.agent.AgentPresentationManager(),
            havenUiBridge = mockk(relaxed = true),
            standingPolicyRepository = mockk(relaxed = true),
            mcpTunnelManager = mockk(relaxed = true),
            mcpStatusHolder = mockk(relaxed = true),
            reticulumSessionManager = mockk(relaxed = true),
            reticulumForwardServer = mockk(relaxed = true),
            mailRuleRepository = mockk(relaxed = true),
            mailWatchManager = mockk(relaxed = true),
            agentActivityHolder = mockk(relaxed = true),
        )
        return tools to proot
    }

    private fun writeArgs(content: String, append: Boolean? = null, final: Boolean? = null) =
        JSONObject().put("path", targetPath).put("content", content).apply {
            if (append != null) put("append", append)
            if (final != null) put("final", final)
        }

    @Test
    fun `two ordered chunks commit once with concatenated bytes`() = runTest {
        val (tools, proot) = newTools()

        val r1 = tools.call("write_guest_file", writeArgs("AAA", append = false, final = false))
        assertEquals("buffered", r1.getString("status"))
        assertEquals(3, r1.getInt("bufferedBytes"))
        // Nothing copied into the guest yet.
        coVerify(exactly = 0) { proot.runCommandInProot(any()) }

        val r2 = tools.call("write_guest_file", writeArgs("BBB", append = true, final = true))
        assertEquals("written", r2.getString("status"))
        assertEquals(6, r2.getInt("bytesWritten"))

        coVerify(exactly = 1) { proot.runCommandInProot(any()) }
        assertEquals("AAABBB", committed.single().toString(Charsets.UTF_8))
        // Staging buffer cleaned up after commit.
        assertEquals(false, stagingFile().exists())
    }

    @Test
    fun `single call without chunk params writes immediately (regression)`() = runTest {
        val (tools, proot) = newTools()

        val r = tools.call("write_guest_file", writeArgs("hello"))

        assertEquals("written", r.getString("status"))
        assertEquals(5, r.getInt("bytesWritten"))
        coVerify(exactly = 1) { proot.runCommandInProot(any()) }
        assertEquals("hello", committed.single().toString(Charsets.UTF_8))
    }

    @Test
    fun `append=false truncates a stale buffer from an abandoned transfer`() = runTest {
        val (tools, proot) = newTools()

        // An abandoned transfer left bytes buffered (final never sent).
        tools.call("write_guest_file", writeArgs("STALE", append = false, final = false))
        coVerify(exactly = 0) { proot.runCommandInProot(any()) }

        // A fresh transfer to the same path starts clean (append=false).
        tools.call("write_guest_file", writeArgs("NEW", append = false, final = true))

        coVerify(exactly = 1) { proot.runCommandInProot(any()) }
        assertEquals("NEW", committed.single().toString(Charsets.UTF_8))
    }
}
